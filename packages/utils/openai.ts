...
export class OpenAIService {
  ...
  async generateVibeSummary(
    ...
    try {
      const prompt = this.buildVibePrompt(
        displayName,
        topArtists,
        topGenres,
        currentTrack
          ...
          messages: [
            {
              role: 'system',
              content:
                "You are a poetic music curator who captures the emotional essence of someone's musical taste in beautiful, evocative language. Create short, soulful summaries that feel like glimpses into someone's inner world.",
            },
            {
              role: 'user',
              content: prompt,
      ...
      return (
        ...
        `${displayName} moves through soundscapes of ${
          topGenres[0] || 'infinite possibility'
        }, finding beauty in every beat.`
      );
    } catch (error) {
  ...
  private buildVibePrompt(
    ...
    if (topArtists.length > 0) {
      prompt += `Their top artists include ${topArtists
        .slice(0, 3)
        .join(', ')}. `;
    }

    if (topGenres.length > 0) {
      prompt += `They gravitate toward ${topGenres
        .slice(0, 2)
        .join(' and ')} music. `;
    }
  ...
  async generatePlaylistDescription(
    ...
    try {
      const prompt = `Create a poetic description for a playlist containing these tracks: ${tracks.join(
        ', '
      )}. ${
        mood ? `The mood is ${mood}.` : ''
      } Write something beautiful and evocative in 1-2 sentences.`;
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a poetic music curator. Create beautiful, emotional descriptions for playlists.',
            },
            {
              role: 'user',
              content: prompt,
