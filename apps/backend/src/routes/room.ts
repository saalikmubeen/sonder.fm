...
// Get room history
router.get('/:roomId/history', auth, async (req: AuthRequest, res) => {
  try {
...
router.post('/:roomId/export-playlist', auth, async (req: AuthRequest, res) => {
  try {
    const { roomId } = req.params;
    ...
    const playlistDescription = description || `Exported from Sonder.fm room "${room.name}" on ${new Date().toLocaleDateString()}`;

    const createPlaylistResponse = await fetch(`https://api.spotify.com/v1/users/${user.spotifyId}/playlists`, {
      method: 'POST',
      headers: {
    ...
    const trackUris = room.songHistory.map(song => `spotify:track:${song.trackId}`);
    const batchSize = 100;
...
router.get('/:roomId/details', auth, async (req: AuthRequest, res) => {
  try {
    const { roomId } = req.params;
    const userId = req.userId;
    ...
    if (userId) {
      ...
      if (user && (user as any).following) {
        const followingIds: string[] = (user as any).following.map((id: any) => id.toString());
        hasFriends = room.participants.some((p: any) => followingIds.includes(p._id.toString()));
      }
    }
